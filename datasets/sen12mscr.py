...
#     s1_lst, s2_lst, s2c_lst = [], [], []

#     for path in tqdm(self.paths):
#         s1 = torch.from_numpy(process_SAR(read_img(read_tif(os.path.join(self.root_dir, path['s1'])))))
#         s2 = torch.from_numpy(process_SAR(read_img(read_tif(os.path.join(self.root_dir, path['s2'])))))
#         s2c = torch.from_numpy(process_SAR(read_img(read_tif(os.path.join(self.root_dir, path['s2_cloudy'])))))

#         s1_lst.append(s1)
#         s2_lst.append(s2)
#         s2c.append(s2c)
    
#     cache = {"s1": s1_lst, "s2": s2_lst, "s2_cloudy": s2c_lst}
#     torch.save(cache, self.cache_path)
#     print("Saved cache to", self.cache_path)
#     return cache
...
class SEN12MSCR(Dataset):
    def __init__(self, root, split="all", region='all', sample_type='pretrain'):
        ...
        # self.cache_path = os.path.join(self.root_dir, self.cache_name + f"_{256}.pth")
