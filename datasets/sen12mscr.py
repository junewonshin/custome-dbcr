...
class SEN12MSCR(Dataset):
    ...
    def __getitem__(self, pdx):  # get the triplet of patch with ID pdx
        ...
        s1              = torch.from_numpy(process_SAR(read_img(s1_tif)))
        s2              = torch.from_numpy(process_MS(read_img(s2_tif)))
        s2_cloudy       = torch.from_numpy(process_MS(read_img(s2_cloudy_tif)))
        # sample = {'input': {'S1': s1,
        #                     'S2': s2_cloudy,
        #                     },
        #             'target': {'S2': s2}
        #             }
        return s2, s2_cloudy, s1
