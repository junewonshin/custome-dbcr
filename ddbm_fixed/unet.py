...
class MBConv(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, expansion_factor=2, dims=2):
        super().__init__()
        ...
        self.project = nn.Sequential(
            conv_nd(dims, self.expanded_channels, out_channels, 1, bias=False),
...
class NAFBlock(TimestepBlock):
    def __init__(
        ...
        # kernel = 3, padding = 1 to kernel = 1
...
class SFFusion(nn.Module):
    ...
    def _forward(self, x_opt, x_sar):
        ...
        x_opt = self.opt_norm(x_opt)
        x_sar = self.sar_norm(x_sar)

        q = self.q_proj(x_opt)
        k = self.k_proj(x_sar)
        v = self.v_proj(x_sar)

        q = q.reshape(B, HN, HD, HW).permute(0, 1, 3, 2)
        k = k.reshape(B, HN, HD, HW).permute(0, 1, 3, 2)
        v = v.reshape(B, HN, HD, HW).permute(0, 1, 3, 2)

        q_t = q.transpose(-2, -1)  # (B, num_heads, head_dim, HW)
        
        chunk_size = 4096
        attn_chunks = []
        
        for i in range(0, q_t.size(-1), chunk_size):
            q_chunk = q_t[..., i:i+chunk_size]
            k_chunk = k[..., i:i+chunk_size, :]
            
            attn_chunk = torch.matmul(q_chunk, k_chunk) * (HD ** -0.5)
            attn_chunk = attn_chunk - attn_chunk.max(dim=-1, keepdim=True)[0]
            attn_chunk = F.softmax(attn_chunk, dim=-1)
            
            attn_chunks.append(attn_chunk)
        
        z_chunks = []
        for chunk_idx, i in enumerate(range(0, q_t.size(-1), chunk_size)):
            v_chunk = v[..., i:i+chunk_size, :]
            attn_chunk = attn_chunks[chunk_idx]  # Use correct chunk index
            z_chunk = torch.matmul(v_chunk, attn_chunk)
            z_chunks.append(z_chunk)

        z = torch.cat(z_chunks, dim=-1)
        z = z.permute(0, 2, 1, 3).reshape(B, HW, C)  # concat heads → (B, HW, C), C = num_heads * head_dim
...
class NAFUNetModel(nn.Module):
    def __init__(
        ...
        self.time_embed_dim = model_channels * 2
        self.time_embed = nn.Sequential(
        ...
        # input conv -> c * 22 * 3 * 3
        self.opt_input_conv = TimestepEmbedSequential(conv_nd(2, in_channels, model_channels, 1))  # 13 -> 22
        self.sar_input_conv = TimestepEmbedSequential(conv_nd(2, 2, model_channels, 1))  # 2 -> 22
        ...
        self.out = conv_nd(2, ch, out_channels, 3, padding=1)
