...
class TrainLoop:
    ...
    def _load_and_sync_parameters(self):
        ...
        if resume_checkpoint:
            ...
            if dist.get_rank() == 0:
                ...
                logger.log("Resume epoch: ", self.resume_step)
    ...
    def _load_optimizer_state(self):
        ...
        if main_checkpoint.split("/")[-1].startswith("freq"):
            ...
            prefix = ""
        opt_checkpoint = bf.join(bf.dirname(main_checkpoint), f"{prefix}opt_{self.resume_step:03}.pt")
    ...
    def run_loop(self):
        while True:
            for batch, cond, sar, pdx in self.data:
                if not (not self.lr_anneal_steps or self.step < self.total_training_steps):
                    if (self.step - 1) % self.save_interval_epochs != 0:
                        self.save()
                    return
                ...
                if took_step and self.epoch % self.test_interval_epochs == 0 and self.step % self.steps_per_epoch == 0:
                    ...
                    test_batch, test_cond, test_sar, test_pdx = next(iter(self.test_data))
            ...
            for name, p in self.model.named_parameters():
                with torch.no_grad():
                    ...
                    if p.grad is not None:
                        if torch.all(torch.isfinite(p.grad)):
                            grad_norm += torch.norm(p.grad, p=2, dtype=torch.float32).item() ** 2
                        else:
                            print(name, "p.grad is NaN")
                            NaN_detected = True
            return np.sqrt(grad_norm), np.sqrt(param_norm), NaN_detected

        grad_norm, param_norm, skip_maker = _compute_norms()
    ...
    def forward_backward(self, batch, cond, train=True):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            if not torch.isfinite(loss) or not all(torch.isfinite(v.float()).all() for v in losses.values()):
                print("NaN Detected in loss dict ")
                return 
...
def find_ema_checkpoint(main_checkpoint, step, rate):
    ...
    if main_checkpoint.split("/")[-1].startswith("freq"):
        ...
        prefix = ""
    filename = f"{prefix}ema_{rate}_{(step):03d}.pt"
